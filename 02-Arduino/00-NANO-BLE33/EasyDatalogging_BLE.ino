...
uint8_t v;                    // The message I am sending from my central
...
void onTriggerWritten();    // Event Handler (RTOS)
...
void setup() {
  ...
  if (WITH_BLUETOOTH == 1) {
    initializeBLE();
    triggerChar.setEventHandler(BLEWritten, onTriggerWritten); // Sets the Event Handler, like an ISR but not hardware driven, involved in the RTOS
...
void initializeBLE() {
  ...
  while(!BLE.connected()) { yield(); }
  central = BLE.central();              // Now we can operate methods for this object Central (my phone)
  v = 0;                                // Initialize the message
}
...
void onConnect(BLEDevice central) {
  bleConnected = true;               // set a flag; keep handler tiny
}

// Event Handler, BLE
void onDisconnect(BLEDevice central) {
  bleConnected = false;
  BLE.advertise();                   // ensure we go back to advertising
}
...
void calibrateIMU2() {
  ...
  if (CALIB_SAFETY_CHECK == 1) {
    ...
    if (j < 300 || fullSpanCalibration(xMin, xMax, yMin, yMax, zMin, zMax)) {
      retryCalibration = 1;
    }
  }
